...
In this case, we assume no measurement error on $y$ such that $\epsilon_{y} = 0$. Consequently, $y$ becomes a prefect indicator of $\eta$ such that the reliability is total and $\lambda_{y} = 1$. We will get the calculation of $\zeta$ momentarily, which involves the value of the path(s) leading into $\eta$.
...
We can solve the path coefficient $\gamma$ by knowing the correlation (coefficient) between the raw values of $x$ and $y$ and adjusting by the loading of $x$ on $\epsilon$.

Let's return to our previous example and generate some data for $y$, then estimate the (standardized) coefficient:
...
y.list <- lapply(1:5, function(i) y + runif(10, 0, 10))
...
In this example, the estimated standardized path coefficient for $x$ on $y$ is $b = 0.182$.
...
So the new estimate of the coefficient between the two latent variables is $\gamma = 0.204$. From this, we can obtain the unexplained variance, or $\epsilon$. Recall that the error $\delta_{x}$ is 1 - the explained variance, where the explained variance is the reliability. Here, we can transfer this knowledge such that: $\epsilon = 1 - \gamma^2$:
...
The error variance based on the model incorporated measurement error in $x$ has decreased from 0.967 to 0.959. By incorporating the error in $x$ into our model, we have improved our estimate of the relationship between $x$ and $y$ *and* decreased the unexplained variance.

## 1.3 Latent Variables in *lavaan*

Let's reproduce this example using *lavaan*. The setup is almost identical except for a new operator `=~` which indicates a latent variable. Additionally, we will fix the error variance in $x$ to the known rror variance from our repeated trials $1 - r = 0.196$.
...
The standardized loading on $xi = 0.872$ which is very close to the value we calculated $\sqrt(r) = 0.897$. The loading on $\eta$ is $\lambda_{y} = 1$. Notice how we didn't specify that: the default in *lavaan* is to set the first loading to 1 when the error variance is not supplied (more on this later). 

With respect to the regression coefficient, *lavaan* returned $\gamma = 0.202$ while we obtained $\gamma = 0.204$. Very close! Similarly the error variance on $\eta$ is 0.959, which is also identical to $1 - \gamma^2 = 0.959$. Naturally, then, the explained variances are also nearly identical, being 1 - error variance.
...
We know from our "Rules of Path Coefficients" that the correlation equals the sum of the direct and indirect pathways (Rule 9). The only path connecting $x1$ and $x2$ is through $\epsilon$, thus value of the compound path is the product of the two individual pathways (Rule 3). Thus, the correlation $r_{x1,x2} = lambda_{1} \times \lambda_{2}$. Given the assumption that the two loadings are equal, $r_{x1,x2} = \lambda^2$ and thus $\lambda = \sqrt(r_{x1,x2})$.

We can scale this procedure out for >2 indicators by setting only 2 loadings to be equal: this will give us the necessary information (along with Rule 8 of path coefficients) to generate unique solutions for the other loadings. It is for this reason that at least three indicators are preferred for multi-indicator latent variables: it lessens the impact of the assumption that two loadings are equal.

An alternate solution is to fix one of the loadings to be 1. If, for example, we fix $\lambda_{1} = 1$ then we know that $\lambda_{2} = r_{x1,x2} / \lambda_{1} = r_{x1,x2} / 1 = r_{x1,x2}$. 

Because it is unmeasured, we also need to provide a scale for our multi-indicator latent variable. This can be done by fixing the variance $\zeta = 1$ or by fixing one of the unstandardized loadings to 1. Both accomplish the same objective.

Finally, we can obtain an estimate of reliability from multi-indicator latent variables using the following equation:
...
Here, the error variance of $x$ has automatically been fixed to 0 and the loading to 1.

The notable differences is that, unlike the previous latent variable model, the error variance on $\eta$ has not changed when incorporating measurement error on $y$:
```{r}
summary(xy_model)$r.squared 

inspect(latent_model1, "r2") # increased

inspect(latent_model2, "r2") # unchanged
```

This is because $\zeta = 1 - gamma^2$ and, in this case, $\gamma$ remains unchanged from the correlation between $x$ and $y$ because we have fixed $\lambda_{x1} = 1$. 


You'll also note that the unstandardized estimate of $\gamma$ is unchanged in the second model
