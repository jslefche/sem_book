...
* create dummary variables for each level: this is procedurally the same as above (splitting levels into *k* - 1 separate variables that occupy 0/1). The key here is not to create *k* variables, to avoid the issue raised above about dependence among predictors. 
...
* for suspected interactions with categorical variables, a multigroup analysis is required. In this case, the same model is fit for each level of the factor, with potentially different coefficients (see: Multigroup Models).
...
![bowen_sem](https://raw.githubusercontent.com/jslefche/jslefche.github.io/master/sem_img/bowen_sem.png)
